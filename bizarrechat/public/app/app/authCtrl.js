...
app.controller('authCtrl', function ($scope, $rootScope, $routeParams, $location, $http, Data) {
     ...
     $scope.doReg = function ()
    {
        ...
        var quickbloxapitoken = localStorage.getItem('quickbloxapitoken');
          $http.post('https://api.quickblox.com/users.json', 
          {
    ...
    $scope.doLog = function ()
    {
        ...
        {
            console.log(email);
            localStorage.setItem('bizarrechat_id', results.data.userid);
            $scope.data.show = true           
            if (results.data.success == 1)
            {
                
           $http({
            url: 'resources/views/layout/calls/createSession.php',
            method: "GET",
            params: {data: results.data.userid}
        }).success(function (status, data, response, header) {
        });
                $location.path('dashboard');
            }
            else if (results.data.success == 3)
            {
    ...
    $scope.logout = function () {
        ...
        $http.post('Logout', {}).then(function (results)
        {
            ...
            {
                $http({
            url: 'resources/views/layout/calls/destroySession.php',
            method: "GET",
            params: {data: results.data.userid}
        }).success(function (status, data, response, header) {
        });
    ...
    $scope.updateProfileName = function ()
    {
        ...
        userId = localStorage.getItem("bizarrechat_id");
        $http.post('updateProfileName', {userProfileName: userProfileName, userId : userId}).then(function (results)
        {
            if (results.data.success == 1)
            {
                console.log('success');
...
$scope.updateContactUs  = function ()
    {
        ...
        $http.post('updateContactUs ', {userId : userId,comments: contactUsComments, type: contactUsType}).then(function (results)
        {
            $scope.data.show = true
            if (results.data == 'success')
            {
                $scope.data.msg = 'Contact details sent succesfully'
                console.log('success');
            }
            else
            {
    ...
    $scope.doSubscribe  = function ()
    {
        ...
        payment_card_type = $scope.payment_card_type;
        payment_package = $scope.payment_package;
        $scope.data.show = false;
        userId  = localStorage.getItem("bizarrechat_id");
        $http.post('doSubscribe', {userId : userId, payment_category : payment_category, payment_card_type: payment_card_type, payment_package: payment_package}).then(function (results)
        {
            $scope.data.show = true
            if (results.data.success == 1)
            {
                $scope.data.msg = 'Subscribed Succesfully'
                console.log('success');
            }
            else
            {
